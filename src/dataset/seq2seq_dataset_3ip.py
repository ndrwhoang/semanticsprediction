    ...
    def convert_sample_to_input(self, samples):
        ...
        for i_sample, (sample_id, sample) in enumerate(samples.items()):
            ...
            for word, label in word_labels.items():
                ...
                node_label[alignment] = [value if value is not None else 10 for value in label]
            node_labels.append(node_label)
            ...
            if len(e_labels) == 0:
                ...
                for word_pair, label in e_labels.items():
                    ...
                    edge_label[(first_alignment, sec_alignment)] = [value if value is not None else 10 for value in label]
...
def dataloader_test(dataset):
    ...
    for i, batch in enumerate(dataloader):
        if i == 3: break
        print('===========')
        input_ids, node_labels, edge_labels = batch
        
        print(input_ids.size())
        print(len(node_labels))
        print([node_label.keys() for node_label in node_labels])
        print(len(edge_labels))
        print([edge_label.keys() for edge_label in edge_labels])
